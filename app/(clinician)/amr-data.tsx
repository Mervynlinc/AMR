...
export default function ClinicianAMRData() {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      <ScreenHeader title="Global AMR Data" subtitle="S. aureus Resistance Rates" />

      <View className="px-4 py-3 bg-white border-b border-gray-200">
        <Text className="text-gray-500 text-xs font-medium uppercase mb-2">Filter by Year</Text>
        <View className="flex-row gap-2">
          {['2021', '2022', '2023', '2024'].map(y => (
            ...
            >
              <Text className={year === y ? 'text-emerald-800 font-bold' : 'text-gray-600'}>{y}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-4 pt-4">
        <View className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
          {amrData.sort((a, b) => b.rate - a.rate).map((ab, idx) => (
            <View key={ab.name} className={`py-3 ${idx < amrData.length - 1 ? 'border-b border-gray-100' : ''}`}>
