    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1 px-4 pt-4">
        <View className="bg-gray-900 rounded-xl p-5 mb-4 shadow-sm">
          ...
          <Text className="text-white text-xl font-bold italic mb-3">
            {report.organism}
          </Text>
        ...
        <Text className="text-gray-800 font-bold text-lg mb-3 mt-2">
          Susceptibility Results
        </Text>
        {report.astResults.length > 0 ? (
          report.astResults.map((ast, idx) => <ASTCard key={idx} entry={ast} />)
        ) : (
          <Text className="text-gray-500 italic mb-4">
            No specific AST record mapping available for this mock.
          </Text>
        )}
