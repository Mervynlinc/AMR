...
import { FileText, ShieldAlert, LogOut, Info } from 'lucide-react-native';
  ...
  return (
    <View className="flex-1">
      <View className="bg-emerald-700 pt-12 pb-4 px-4 flex-row items-center justify-between pt-safe">
        <View>
          <Text className="text-white text-xl font-bold">Welcome, {user?.name}</Text>
      ...
      <ScrollView className="flex-1 px-4 pt-4 bg-gray-50">
        <View className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex-row items-start mb-6">
          ...
          <Text className="text-blue-800 flex-1 text-sm">
            Read-only Access: You are viewing verified lab data. For sample registration, contact your designated laboratory.
          </Text>
        </View>
        ...
        <View className="flex-row gap-4 mb-6">
          <View className="flex-1 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            ...
            <Text className="text-2xl font-bold text-gray-900">{reports.length}</Text>
            <Text className="text-gray-500 text-xs mt-1">Available Reports</Text>
          </View>
          <View className="flex-1 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
        ...
        {isLoading ? (
          ...
          reports.slice(0, 3).map(report => (
            ...
            >
              ...
              <Text className="text-gray-600 mb-1">{report.specimenType} • {report.organism}</Text>
